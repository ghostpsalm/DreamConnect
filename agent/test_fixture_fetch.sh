...
# The same rail test_install.sh and test_build.sh carry. The fetch cases below
# name a real per-box cache path, so a case that forgot its
# DC_FIXTURE_CACHE_DIR override must not be able to run as root against the
# real one.
