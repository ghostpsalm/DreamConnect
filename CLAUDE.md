...
the network sets `DC_BYTEBUDDY_JAR` itself and the fetcher verifies that instead. No test may reach
the network — `agent/test_build.sh` keeps its `curl` stub, and the fetch lives in the runner where it
is visible rather than inside a test helper.
